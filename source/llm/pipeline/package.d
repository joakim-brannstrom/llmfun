...
/// Wrapper that ensures an agent calls pipelineOutput before completing. Implements the retry loop: if node output is empty after a run, re-prompt the agent and retry up to conf.maxRetries times.
class PipelineAgent : IBasicAgent {
    ...
    override ProcessResult runToCompletion(void delegate(ProcessResult) step = null,
        ...
        while (true) {
            ...
            // Retry nudge is harness traffic: continue the current turn instead of opening a new one, and keep it out of the Facts projection. addUserQuery would fragment the node's logical turn.
...
struct Pipeline {
    ...
    /// Configure the number of worker threads for pool-based execution. Default is 1.
    ...
    /// Callback invoked by the pool when an agent finishes execution. This is the heart of the event-driven execution model. All shared state access runs under _mutex to prevent races between pool threads. Pool submissions happen OUTSIDE the mutex to prevent deadlock.
    private void onAgentComplete(PipelineNode node, ProcessResult result) {
...

        {
            ...
            foreach (rn; readyNodes) {
                ...
                // Clear accumulated inputs if this node is being re-executed (loop edge) to prevent stale inputs contaminating the new run
                if (pn.executed) {
            ...
            // Capture signal flag under mutex to avoid reading _pipelineDone outside lock (prevents deadlock if callback re-enters during pool.submit)
            shouldSignal = _pipelineDone;
        }

        // Check interrupt delegate OUTSIDE the mutex before submitting new tasks (prevents deadlock if delegate blocks while holding mutex)
        ...
        // Submit tasks OUTSIDE the mutex to prevent deadlock (callbacks re-enter onAgentComplete which locks the same mutex)
    ...
    /// @deprecated: Replaced by pool-based execution. Retained for potential future use.
    private auto runToCompletion(PipelineNode node) {
        while (true) {
            auto res = node.agent.runToCompletion();
            if (node.output.empty) {
                // Harness retry nudge: continue the current turn, never open a new one (addUserQuery would fragment the turn).
    ...
    /// Build PipelineResult from tracked execution data. Must be called after pipeline completes (under no lock or after unlock).
...
struct PipelineBuilder {
    ...
    /// Set the number of worker threads for pool-based execution. Returns the builder for fluent chaining.
